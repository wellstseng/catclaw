...
### 27. CLI Bridge 圖片附件不能傳 Discord CDN URL（2026-04-15）
原本只把 attachment URL 以文字附加到 stdin，Claude CLI 把 URL 當 image source 送 Anthropic API → 400 "Could not process image"（CDN 權限/過期/型別偵測失敗），session 卡死
修正：`extractAttachments()` 下載 png/jpeg/gif/webp（≤5MB）→ base64 → stdin content 改送 array `[{type:"text"}, {type:"image", source:{type:"base64",...}}]`
降級：非支援格式或下載失敗 → 退回 URL 文字描述

### 28. /clear-session 要同步清 stdout.jsonl + compact turns（2026-04-15）
只清 sessionId 不清日誌 → Dashboard Memory tab 仍顯示舊對話
修正：`clearSessionId()` 內部呼叫 `stdoutLogger.truncateStdout()` + `compactTurns(60)`（舊 turns placeholder、TTL 60 天）
注意：stdout.jsonl 純觀測 log、turns.jsonl 統計，都不注入 CLI context（context 由 `--resume <sessionId>` 維護）
