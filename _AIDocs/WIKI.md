...
multi-turn agent loop、17+ builtin tools、31 builtin skills、多 provider failover、
...
| **Skills** | 31 builtin skills（28 TS + 3 prompt） |
...
### 4.3 Skill 系統（31 builtin skills）

Skill = Discord 指令層，在 agent loop 之前攔截。28 個 TypeScript 執行型 + 3 個 prompt 型。
...
**Recall**（5 步管線）：embed query → LanceDB vector search → 讀取命中 atom → budget 截斷 → 注入 prompt
