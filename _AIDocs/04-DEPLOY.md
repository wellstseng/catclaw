...
# 編輯 config.json：填入 discord.token，設定 guilds 權限
...
PM2 的 `watch` 功能監聽 `signal/` 目錄變更觸發重啟，而非監聽 `dist/`。
好處：tsc 編譯不會立即重啟，開發者可以先編譯確認、取得使用者確認後再觸發重啟。
...
- 監聽機制：`fs.watch(config.json, ...)` + 500ms debounce
...
```bash
# 修改 config.json 後，等約 0.5s 即自動生效
...
修改 `config.json` 中的 `logLevel` 為 `"debug"`，存檔後 hot-reload 自動生效，可看到詳細的訊息過濾流程。
...
1. 確認 `config.json` 的 `guilds` 設定中對應 guildId 的 `allow = true`
...
1. 確認 `config.json` 的 `cron.enabled = true`
