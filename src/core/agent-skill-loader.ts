...
export function buildSkillCreationHint(agentId: string): string {
  ...
  return [
    `\n\n# Skill 自建能力\n`,
    `你可以用 write_file 在 \`${skillsDir}/\` 底下建立新 skill，下次被召喚時自動載入。`,
    `**新建 skill 一律用 folder 格式**（相容 Claude Code SKILL.md 規格，可隨檔附 bundle 腳本／資產）：寫成 \`${skillsDir}/my-skill/SKILL.md\` + 任意配套檔。\n`,
    "```markdown",
    "---",
    "name: my-skill",
    "description: 簡短說明（會出現在 LLM 觸發判斷裡，要精準）",
    ...
    "```\n",
    `_（舊的平鋪 \`${skillsDir}/{name}.md\` 格式 loader 仍能讀，但僅供既有 skill back-compat，不要再用來新建。）_`,
  ].join("\n");
}
