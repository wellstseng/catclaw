...
// ── Soft-timeout：tool 跑超過此時間 → catclaw 主動 abort + 回 actionable error 給 LLM 自行判斷 ──
// per-tool 可在 Tool 物件中 override `softTimeoutMs`。預設依工具屬性分類。
const DEFAULT_SOFT_TIMEOUT_MS_BY_TOOL: Record<string, number> = {
  run_command: 120_000,        // 120s：shell 指令可能跑長腳本
  run_background_command: 0,   // 背景指令不限制
  read_file: 30_000,           // 30s：大檔案可能要時間
  write_file: 30_000,          // 30s
  edit_file: 30_000,           // 30s
  glob: 30_000,                // 30s：跨大目錄掃描
  grep: 30_000,                // 30s
  web_fetch: 60_000,           // 60s：網路慢
  web_search: 30_000,
  spawn_subagent: 0,           // subagent 自帶 timeout，這層不限
  subagents: 0,                // wait/list/status 本身會等
};
const DEFAULT_SOFT_TIMEOUT_MS_GENERIC = 60_000;  // 未列舉的 tool 預設 60 秒

function resolveSoftTimeoutMs(toolName: string, perToolOverride?: number): number {
  if (perToolOverride !== undefined) return perToolOverride;  // 顯式設值（含 0）優先
  return DEFAULT_SOFT_TIMEOUT_MS_BY_TOOL[toolName] ?? DEFAULT_SOFT_TIMEOUT_MS_GENERIC;
}

/**
 * 包裹 tool execute：Promise.race(tool, softTimeoutTimer)。
 * 觸發 soft-timeout → 構造 actionable error 給 LLM 自己決策（continue / 換工具 / end_turn）。
 *
 * 設 softTimeoutMs=0 表示停用 watchdog，直接 await tool。
 *
 * 注意：tool 真正中止依賴 tool 自身是否支援 AbortController/SIGTERM；
 * 不支援的 tool 會「detach」（promise 仍跑，結果被丟），但 LLM 看到的是 timeout 訊息可以繼續。
 */
async function executeToolWithSoftWatchdog(
  ...
  if (softTimeoutMs <= 0) {
    return toolRegistry.execute(toolName, params, ctx);
  }
  const toolPromise = toolRegistry.execute(toolName, params, ctx);
  let timerId: ReturnType<typeof setTimeout> | undefined;
  const timeoutPromise = new Promise<ToolResult>(resolve => {
    timerId = setTimeout(() => {
      resolve({
        error:
          `tool '${toolName}' soft-timeout after ${Math.round(softTimeoutMs / 1000)}s（catclaw 主動中止）` +
          `\n請判斷下一步：` +
          `\n(a) 縮小 scope 重試（例如 read_file 改用 offset/limit 分段；run_command 改短或加 head/tail 限制；glob/grep 縮小路徑/pattern）` +
          `\n(b) 換用更輕量的 tool（例如 read_file 替代 cat；grep 替代複雜的 run_command pipeline）` +
          `\n(c) 若任務需要長時間執行，改用 spawn_subagent 外包` +
          `\n(d) 任務已不可繼續 → end_turn 回報目前進度給 user`,
      });
    }, softTimeoutMs);
  });
  try {
...
export async function* agentLoop(
  ...
  //   Wells 偏好：catclaw 不應該強制中止任務，靠不斷 nudge 引導 agent 自行轉策略
  let stuckLoopBlockCount = 0;
  let stuckLoopSoftNudgeInjected = false;
  let stuckLoopCycleCount = 0;
  const STUCK_LOOP_CYCLE_THRESHOLD = 3;
  // L3 Tool Cluster Detection — subagents.{wait,list,status}（同 cycle 制，無 break）：
  ...
  try {
    while (true) {
      ...
      for (const call of sequentialCalls) {
        ...
        const softTimeoutMs = resolveSoftTimeoutMs(call.name, toolDef?.softTimeoutMs);
        let toolResult = await executeToolWithSoftWatchdog(toolRegistry, call.name, effectiveParams, toolCtx, softTimeoutMs);
        for (let attempt = 1; attempt <= TOOL_TIMEOUT_MAX_RETRIES; attempt++) {
          ...
          toolResult = await executeToolWithSoftWatchdog(toolRegistry, call.name, effectiveParams, toolCtx, softTimeoutMs);
        }
      ...
      // catclaw 反覆引導換策略，極端 safety net（3 cycle 都失敗）才真 break。
      {
        ...
        // STUCK-LOOP：每 cycle 升級 nudge + 重置（無限循環，不 break）
        ...
        // subagent polling：每 cycle 升級 + 重置（無限循環，不 break）
